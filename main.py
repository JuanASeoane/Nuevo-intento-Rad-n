"""
App de toma de datos de detectores de radón
=============================================
Estructura (igual que la app de referencia):

Pantalla inicial - "Centros registrados"
    - Selector de centros grabados + "Abrir centro"
    - Lista de centros (nombre, zona, fecha)
    - "+ Crear nuevo centro"

Pantalla "Centro"
    - Datos del centro: nombre, zona, fecha de la medición, imagen exterior
    - Detectores colocados: lista + "Colocar nuevo detector" + "Editar detectores"
    - Informe y envío: "Generar PDF y enviar por WhatsApp" + selección de fotos a adjuntar

Pantalla "Detector" (Nuevo / Editar)
    - Datos del detector: planta, sala, fecha, código
    - Plano de situación del detector (con marcador de punto rojo)
    - Fotografías: imagen de la situación del detector, imagen del detector
...
def init_db():
    ...
    cur.execute(
        ...
        CREATE TABLE IF NOT EXISTS detectores (
            ...
            sala TEXT,
            fecha TEXT,
            detector_codigo TEXT,
            plano_path TEXT,
            punto_x REAL,
            punto_y REAL,
            foto_situacion_path TEXT,
            foto_detector_path TEXT,
            fecha_creacion TEXT
        )
        """
    )
...
def crear_centro(nombre):
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO centros (nombre, zona, fecha_medicion, imagen_exterior_path) VALUES (?, '', ?, NULL)",
...
def get_centro(centro_id):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT id, nombre, zona, fecha_medicion, imagen_exterior_path FROM centros WHERE id=?",
        (centro_id,),
    )
    row = cur.fetchone()
    conn.close()
...
def update_centro(centro_id, nombre, zona, fecha, imagen_exterior_path):
    ...
    cur = conn.cursor()
    cur.execute(
        "UPDATE centros SET nombre=?, zona=?, fecha_medicion=?, imagen_exterior_path=? WHERE id=?",
        (nombre, zona, fecha, imagen_exterior_path, centro_id),
    )
    conn.commit()
    conn.close()
...
def fetch_detectores(centro_id):
    ...
    cur.execute(
        """
        SELECT id, centro_id, planta, sala, fecha, detector_codigo, plano_path,
               punto_x, punto_y, foto_situacion_path, foto_detector_path, fecha_creacion
        FROM detectores WHERE centro_id=? ORDER BY id ASC
        """,
        (centro_id,),
    )
    rows = cur.fetchall()
...
def elegir_archivo(callback, titulo="Selecciona una imagen"):
    """Abre un selector de archivos para elegir una imagen existente
    (equivalente a la galería)."""
    content = BoxLayout(orientation="vertical", spacing=dp(6), padding=dp(6))
    start_path = os.path.expanduser("~")
    chooser = FileChooserIconView(filters=IMG_FILTERS, path=start_path)
...
def tomar_foto_camara(callback):
    """Toma una foto con la cámara del dispositivo (plyer). Si falla o no
    hay cámara disponible, recurre al selector de archivos."""
    data_dir = get_data_dir()
    filename = os.path.join(
        data_dir, f"foto_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}.jpg"
    )
...
def elegir_o_tomar_foto(callback, titulo="Añadir imagen"):
    """Pregunta al usuario si quiere tomar una foto nueva o elegir una
    existente (equivalente al 'Sube o fotografía' de la app original)."""
    ...
    btn_archivo = Button(text="Elegir de galería / archivo", size_hint_y=None, height=dp(46))
...
def copiar_a_datos(origen, prefijo):
    """Copia un archivo de imagen al almacenamiento privado de la app
    para que persista aunque el archivo original se mueva o borre."""
    data_dir = get_data_dir()
    ext = os.path.splitext(origen)[1] or ".jpg"
    destino = os.path.join(
        data_dir, f"{prefijo}_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}{ext}"
    )
...
def abrir_ajustes_popup():
    ...
    content.add_widget(
        Label(
            text="Nombre del técnico / empresa\n(aparecerá en el PDF)",
            size_hint_y=None, height=dp(50),
        )
    )
    input_tecnico = TextInput(
        text=get_setting_tecnico(), multiline=False, size_hint_y=None, height=dp(44)
    )
...
def compartir_archivos(filepaths):
    """Comparte una lista de archivos (PDF + fotos seleccionadas) usando
    el selector nativo de Android, para poder enviarlos por WhatsApp u
    otra app. En escritorio (sin integración de "compartir") se informa
    de la ruta de los archivos generados."""
    filepaths = [f for f in filepaths if f and os.path.exists(f)]
    if platform != "android":
        rutas = "\n".join(filepaths)
        show_popup(
            "Archivos generados",
            f"En un dispositivo Android podrás compartir esto directamente "
            f"por WhatsApp. Archivos listos en:\n{rutas}",
        )
        return
    ...
    def compartir_siguiente(idx):
        ...
        path = filepaths[idx]
        try:
            share.share(title="Compartir informe de radón", filepath=path)
        except Exception as exc:  # noqa: BLE001
            show_popup("Error al compartir", str(exc))
            return
...
class PlanoMarcador(Widget):
    """Muestra una imagen de plano y permite marcar un único punto rojo
    tocando sobre ella (para indicar la ubicación de la medición)."""

    source = StringProperty("")
    norm_x = NumericProperty(-1)  # -1 => sin marcar. Rango válido: 0..1
    norm_y = NumericProperty(-1)  # 0 = arriba de la imagen, 1 = abajo
...
def _componer_plano_con_marca(plano_path, norm_x, norm_y, output_path):
    from PIL import Image as PILImage, ImageDraw

    img = PILImage.open(plano_path).convert("RGB")
    w, h = img.size
    draw = ImageDraw.Draw(img)
    if norm_x is not None and norm_y is not None and norm_x >= 0 and norm_y >= 0:
        px, py = norm_x * w, norm_y * h
        r = max(6, int(min(w, h) * 0.018))
        draw.ellipse((px - r, py - r, px + r, py + r),
                      fill=(214, 30, 30), outline=(110, 0, 0), width=max(1, r // 4))
    img.save(output_path, quality=90)
    return output_path


def _imagen_ajustada(path, max_w, max_h):
...
def generar_pdf_centro(centro_id, output_path):
    """Genera el PDF del informe de un centro: datos del centro, imagen
    exterior, y por cada detector: datos, plano con el punto marcado y
    fotos."""
    ...
    _, nombre, zona, fecha_medicion, imagen_exterior = centro
    ...
    story = []
    story.append(Paragraph("Informe de medición de radón", styles["Title"]))
    story.append(Spacer(1, 0.3 * cm))
    story.append(Paragraph(f"<b>Centro:</b> {nombre or '-'}", styles["Normal"]))
    if zona:
        story.append(Paragraph(f"<b>Zona:</b> {zona}", styles["Normal"]))
    story.append(Paragraph(f"<b>Fecha de la medición:</b> {fecha_medicion or '-'}", styles["Normal"]))
    if tecnico:
        story.append(Paragraph(f"<b>Técnico:</b> {tecnico}", styles["Normal"]))
    story.append(Paragraph(f"<b>Número de detectores:</b> {len(detectores)}", styles["Normal"]))
    story.append(Spacer(1, 0.5 * cm))

    if imagen_exterior and os.path.exists(imagen_exterior):
        story.append(Paragraph("Imagen exterior del centro:", styles["Heading4"]))
        story.append(_imagen_ajustada(imagen_exterior, 14 * cm, 9 * cm))
        story.append(Spacer(1, 0.5 * cm))

    if not detectores:
        story.append(Paragraph("No hay detectores registrados todavía.", styles["Normal"]))

    temp_dir = os.path.join(get_data_dir(), "_pdf_tmp")
    os.makedirs(temp_dir, exist_ok=True)
    max_img_w = 16 * cm

    for idx, det in enumerate(detectores, start=1):
        (did, _centro_id, planta, sala, fecha, codigo, plano_path, punto_x, punto_y,
         foto_sit, foto_det, fecha_creacion) = det
        ...
        if plano_path and os.path.exists(plano_path):
            story.append(Paragraph("Plano con ubicación marcada:", styles["Heading4"]))
            marcado_path = os.path.join(temp_dir, f"plano_marcado_{idx}.jpg")
            try:
                _componer_plano_con_marca(plano_path, punto_x, punto_y, marcado_path)
                story.append(_imagen_ajustada(marcado_path, max_img_w, 10 * cm))
            except Exception as exc:  # noqa: BLE001
                story.append(Paragraph(f"(No se pudo procesar el plano: {exc})", styles["Normal"]))
            story.append(Spacer(1, 0.4 * cm))

        if foto_sit and os.path.exists(foto_sit):
            story.append(Paragraph("Foto de la situación del detector:", styles["Heading4"]))
            story.append(_imagen_ajustada(foto_sit, 9 * cm, 9 * cm))
            story.append(Spacer(1, 0.4 * cm))

        if foto_det and os.path.exists(foto_det):
            story.append(Paragraph("Foto del detector:", styles["Heading4"]))
            story.append(_imagen_ajustada(foto_det, 9 * cm, 9 * cm))
...
class HomeScreen(Screen):
    ...
    def _cargar(self):
        ...
        if not centros:
            container.add_widget(
                Label(
                    text="Todavía no hay centros registrados.",
    ...
    def crear_centro_popup(self):
        content = BoxLayout(orientation="vertical", spacing=dp(10), padding=dp(14))
        content.add_widget(
            Label(text="Nombre del nuevo centro", size_hint_y=None, height=dp(28))
        )
...
class CentroScreen(Screen):
    ...
    def _cargar(self):
        ...
        self.ids.editar_detectores_btn.text = "Ocultar edición" if self.modo_edicion else "Editar detectores"
    ...
    def _cargar_detectores(self):
        ...
        if not detectores:
            container.add_widget(
                Label(
                    text="Todavía no hay detectores colocados.",
                    size_hint_y=None, height=dp(36),
                    color=(0.4, 0.4, 0.4, 1),
                )
            )
        for d in detectores:
            (did, _centro_id, planta, sala, fecha, codigo, plano_path, px, py,
             foto_sit, foto_det, fecha_creacion) = d
            ...
            if self.modo_edicion:
                btn_borrar = Button(
                    text="Borrar", size_hint_x=None, width=dp(80),
    ...
    def _agregar_check_foto(self, container, etiqueta, path):
        ...
        fila.add_widget(chk)
        lbl = Label(
            text=etiqueta, halign="left", valign="middle", color=(0.1, 0.1, 0.1, 1),
        )
    ...
    def generar_pdf_y_compartir(self):
        if not fetch_detectores(self.centro_id):
            show_popup("Sin datos", "Este centro todavía no tiene detectores colocados.")
            return
        try:
            centro = get_centro(self.centro_id)
            ...
            generar_pdf_centro(self.centro_id, output_path)
        except Exception as exc:  # noqa: BLE001
...
class DetectorScreen(Screen):
    ...
    def adjuntar_plano(self):
        ...
        elegir_o_tomar_foto(al_elegir, titulo="Plano de situación del detector")
    ...
    def foto_situacion(self):
        ...
        elegir_o_tomar_foto(al_tomar, titulo="Foto de la situación del detector")
    ...
    def guardar_y_salir(self):
        ...
        if not sala:
            show_popup("Falta información", "Indica la sala de la medición.")
            return
        if not codigo:
            show_popup("Falta información", "Indica el código del detector.")
            return
        ...
        data = (
            self.centro_id, planta, sala, fecha, codigo, self._plano_path,
            punto_x, punto_y, self._foto_situacion_path, self._foto_detector_path,
